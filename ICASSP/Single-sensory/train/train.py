...
def train(args, device, train_loader, traintest_loader, test_loader):
    ...
    for trial in range(1,args.trials+1):        # 1个100个epochs，实验中只有1（因为默认100epochs）
        # Network topology
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            train_epoch(args, model, device, train_loader, optimizer, loss,onoffnoise=args.train_noise, epochid=epoch)
            scheduler.step()
...
def train_epoch(args, model, device, train_loader, optimizer, loss, onoffnoise, epochid):
    model.train()
    ...
    if args.dataset == 'tidigits' or args.dataset=='MNIST':
        ...
        for batch_idx, (data, label) in enumerate(tqdm(train_loader)):
            ...
            optimizer.zero_grad()
            output = model(data, label, max_len, onoffnoise, args.dataset, epochid,'train',args) # cx：后面的loss计算、backward、step什么的你自己补上
            loss_val = loss[0](output, loss[1](label))
            loss_val.backward(retain_graph=True)
            optimizer.step()
            if args.mini:
...
#        filemotif = writefile(args, '/trainmotif.txt')               ##############################
...
#            filemotif.write(str(epoch) + ' ' + str(motif_mask) + '\n')                          ###################################
        if phase == 'Test':
