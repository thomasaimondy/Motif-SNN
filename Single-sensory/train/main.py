...
def filedel(filepath):
    for i in ['/testloss.txt','/testacc.txt','/trainloss.txt','/trainacc.txt']:
        try:
            os.remove(filepath+i)       # 删除文件
        except:
            pass
def mkd(args):
    if not os.path.isdir('output/' + args.codename.split('-')[0]+'/'+args.codename):    # 判断是否为目录
        os.makedirs('output/' + args.codename.split('-')[0]+'/'+args.codename)      # 创建目录
    if not os.path.isdir('model/' + args.codename.split('-')[0] + '/' + args.codename):
        os.makedirs('model/' + args.codename.split('-')[0] + '/' + args.codename)

def main():
    parser = argparse.ArgumentParser(description='Training fully-connected and convolutional networks using backpropagation (BP), feedback alignment (FA), direct feedback alignment (DFA), and direct random target projection (DRTP)')        # 创建解析器，将命令行解析成Python数据类型
    # General
    parser.add_argument('--cpu', action='store_true', default=False, help='Disable CUDA and run on CPU.')       # 添加参数
    # Dataset
    ...
    parser.add_argument('--codename', type=str, default='test') #CASNNTIDbp4-3
    ...
    parser.add_argument('--gpu',type=str,default='3',help='(default=%(default)s)')      ##################### device: GPU:01
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu       # 使用指定GPU，设置占用每个GPU的比例
