...
class NetworkBuilder(nn.Module):
    ...
    def __init__(self, topology, input_size, input_channels, label_features, train_batch_size, train_mode, dropout,
        ...
        topology = topology.split('_')      # _字符分割
        ...
        for i in range(num_layers):
            ...
            try:
                if layer[0] == "CONV":
                    in_channels = input_channels if (i == 0) else out_channels    # 第一次卷积则为输入通道数，否则为上一层输出通道数
                    ...
                    output_dim = int((input_dim - int(layer[2]) + 2 * int(layer[4])) / int(layer[3])) + 1   # CONV_{output channels}_{kernel size}_{stride}_{padding}
        ...
        self.vote = nn.Linear(in_features=int(layer[1]), out_features=label_features, bias=True)        # Linear全连接

    def add_noise(self, x):
        prop = self.propnoise
        # print((torch.rand(1)-0.5))
        mix = x * (1 + torch.sign(torch.rand(x.shape[0],x.shape[1])-0.5) * prop).cuda()     # rand均匀分布[0,1)################
    ...
    def forward(self, input, labels, spike_window_1014, onoffnoise, dataset,epochid=-1,mode='test',args=None):
        ...
        for step in range(spike_window_1014):# cx：对于timit来说，spike_window换成数据第二维的长度input.size()[1]，虚拟时间和数据的真时间标齐
            if self.topology[0] == 'C':
                x = input[:,:,:,step]           # x是窗中每个时间点的输入
            elif self.topology[0] == 'CONV': # CONV
                if dataset=='tidigits' or dataset == 'MNIST':
                    x = input.float().unsqueeze(1)# {batch_size}{binds}{time_window}, global time. unsqueeze(1)：在第二维增加一个维度
                    #x = 10 * x
                    x = x > torch.rand(x.size()).float().cuda() * self.randKill     # randKill：输入数据编码超参,x大于后面的得True
...
class ActFun(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        return input.gt(spike_args['thresh']).float()       # 高于阈值取1，其余0

    @staticmethod
    def backward(ctx, grad_output):
        input, = ctx.saved_tensors
        grad_input = grad_output.clone()
        temp = abs(input - spike_args['thresh']) < spike_args['lens']       # lens：LIF参数
...
class RFC_block(nn.Module): # cx：你关注下self.rec和self.rec_mask在哪里被定义和引用。这样写的话，FC层在时间上展开成一个RNN了，写model要用RNN的时候直接调这个就行，可以改个名字啥的，不过目前比不上LSTM复杂
    def __init__(self, in_features, out_features, bias, activation, dropout, dim_hook, label_features, fc_zero_init,
        ...
        self.xishuada = xishuada
        self.propnoise = propnoise
        self.fc = nn.Linear(in_features=in_features, out_features=out_features, bias=bias)      # Linear全连接
        self.rec = nn.Linear(in_features=out_features, out_features=out_features)
        if fc_zero_init:
            torch.zero_(self.fc.weight.data)        # fill self Tensors with zeros
        if train_mode == 'FA':
        ...
        self.sumspike = None
        self.time_counter = 0
        self.rec_mask = (1 - torch.eye(self.out_features)).reshape(-1,)     # eye:生成对角线全1其余0数组   并成一行
        self.rec_mask = self.rec_mask.cpu().numpy()
        np.random.seed(None)
        indices = np.random.choice(np.arange(self.rec_mask.size), replace=False, size=int(self.rec_mask.size * 0.5))    # 在self.rec_mask.size中随机采样采一半
        self.rec_mask[indices] = 0          # 这一半变成了0
        self.rec_mask = torch.from_numpy(self.rec_mask).cuda().reshape(out_features,out_features)       # torch.from_numpy完成numpy到tensor数组的转换，rec_mask:0-1之间很多随机采样数
...
class MFC_block(nn.Module):
    def __init__(self, in_features, out_features, bias, activation, dropout, dim_hook, label_features, fc_zero_init,
        ...
        self.sumspike = None
        self.time_counter = 0
        # self.motif_mask = (1 - torch.eye(self.out_features)).reshape(-1, )  # eye:生成对角线全1其余0数组   并成一行
        # self.motif_mask = self.motif_mask.cpu().numpy()
        # np.random.seed(None)        # None为取消随机种子设定
        # indices = np.random.choice(np.arange(self.motif_mask.size), replace=False, size=int(self.motif_mask.size * 0.5))    # 在self.motif_mask.size中随机采样采一半
        # self.motif_mask[indices] = 0          # 这一半indices对应位置的mask变成了0
        # self.motif_mask = torch.from_numpy(self.motif_mask).cuda().reshape(out_features,out_features)  # torch.from_numpy完成numpy到tensor数组的转换，rec_mask:很多随机采样数
        self.motif_mask = torch.zeros((out_features, out_features)).cuda()      # 全0矩阵开始
#        self.motif_mask = torch.randn((out_features, out_features)).cuda()    # 随机矩阵开始
#        self.motif_mask = torch.ones((out_features, out_features)).cuda()  # 全1矩阵开始
...
#        self.cnt = 0        #############################
    ...
    def mem_update(self, x, labels, y, onoffnoise):
        ...
        mem_ff = self.mem * spike_args['decay'] * (1. - self.spike) + self.fc(
            mixvoice) - self.adapThreshold * xishuada  # shuncheng
        spike_ff = self.hook_ff(act_fun(mem_ff), labels, y)
        if self.rec:
            # mask = torch.sigmoid(self.motif_mask)
            # self.rec.weight.data = self.rec.weight * mask  # 增加mask，稀疏连接
    ...
    def forward(self, x, labels, y, spike_window_1014, onoffnoise, epochid, mode, args):
        ...
        # self.motif_mask = (1 - torch.eye(self.out_features)).reshape(-1,)
        # self.motif_mask = self.motif_mask.cpu().numpy()
        # np.random.seed(0)
        # indices = np.random.choice(np.arange(self.motif_mask.size), replace=False, size=int(self.motif_mask.size * 0.5))
        # self.motif_mask[indices] = 0
        # self.motif_mask = torch.from_numpy(self.motif_mask).cuda().reshape(self.out_features,self.out_features)
        if self.time_counter == 0:
            if self.count % 120 == 0 and mode=='train' and (epochid % 10 == 0 or epochid == 1):        # self.count:记录一个epoch之内到第几个batch了（1200个中每120个记录一次，一个epoch内共记录10次）
...
#                print('*'*20)
#                trainmotif = open('mask/{}/MNISTMotif.txt'.format(args.codename), 'a')
#                trainmotif = writefile(self, '/TIDmotif.txt')
#                trainmotif.write(str(epochid) + ' ' + str(self.count) + ' ' + str(self.rec.dmask) + '\n')
            self.count += 1
            self.mem = torch.zeros((x.shape[0], self.out_features)).cuda()
            self.spike = torch.zeros((x.shape[0], self.out_features)).cuda()
            self.sumspike = torch.zeros((x.shape[0], self.out_features)).cuda()
...
class MyLinear(nn.Module):
    ...
    def __init__(self,in_dim,out_dim,generate_mask_method,bias=True):
        ...
        self.pop_volt = torch.zeros(in_dim, out_dim)#, device=self.device)
    ...
    # def normal_sigmoid(self,cmask):
    #     y = F.sigmoid(cmask)
    #     y_hard = torch.bernoulli(y).float()
    #     return y_hard

    # def forward(self,x,cmask,deterministic):
    #
    #     if deterministic==False:
    #         dmask = self.gumbel_sigmoid(cmask)
    #     else:
    #         if METHOD == 'Normal':
    #             dmask = self.normal_sigmoid(cmask)
    #         elif METHOD == 'Gumbel':
    #             dmask = self.gumbel_sigmoid(cmask)
    #     x = torch.matmul(x,(self.W*dmask)) + self.bias
    #
    #     return x
...
class CNN_block(nn.Module):
    ...
    def mem_update(self, ops, x, mem, spike, lateral=None):
        a = ops(x)
        mem = mem * spike_args['decay'] * (1. - spike) + ops(x)     # decay: LIF参数
        # print(mem.gt(thresh).sum())
...
class FC_block(nn.Module):
    def __init__(self, in_features, out_features, bias, activation, dropout, dim_hook, label_features, fc_zero_init,
        ...
        self.act = Activation(activation)
        if dropout != 0:
            self.drop = nn.Dropout(p=dropout)       # 全连接层中减少过拟合，p为中场休息的神经元的比例
        self.hook = TrainingHook(label_features=label_features, dim_hook=dim_hook, train_mode=train_mode)
        self.mem = None
        self.spike = None
        self.sumspike = None
...
class Activation(nn.Module):            # 激活函数
