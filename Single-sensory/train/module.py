...
class FA_wrapper(nn.Module):
    def __init__(self, module, layer_type, dim, stride=None, padding=None):
        ...
        self.fixed_fb_weights = nn.Parameter(torch.Tensor(torch.Size(dim))) # 经类型转换self.fixed_fb_weights变成了模型的一部分，成为模型中根据训练可改动的参数了
        self.reset_weights()
    ...
    def reset_weights(self):
        torch.nn.init.kaiming_uniform_(self.fixed_fb_weights)      # 权重均匀分布初始化
        self.fixed_fb_weights.requires_grad = False
    
    def FA_hook_pre(self, grad):
        if self.output_grad is not None:
            if (self.layer_type == "fc"):
                return self.output_grad.mm(self.fixed_fb_weights)   # output_grad与fixed_fb_weights相乘
...
class TrainingHook(nn.Module):
    ...
    def reset_weights(self):
        torch.nn.init.kaiming_uniform_(self.fixed_fb_weights)       # 凯明初始化，权重均匀分布
        self.fixed_fb_weights.requires_grad = False
