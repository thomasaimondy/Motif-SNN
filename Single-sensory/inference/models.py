        ...
        for step in range(spike_window_1014):# cx：对于timit来说，spike_window换成数据第二维的长度input.size()[1]，虚拟时间和数据的真时间标齐
            if self.topology[0] == 'C':
                ...
                if dataset=='tidigits' or dataset == 'MNIST':
                    ...
                    #x = 10 * x
                    prop = self.propnoise  #########################
#                    x = x * (1 + torch.sign(torch.rand(x.size()) - 0.5) * prop).cuda()  ##################################乘性
                    x = x.cuda() + (((torch.rand(x.size()) - 0.5) * 0.2) * prop).cuda()  ##################################TIDigits加性
#                    x = x.cuda() + (((torch.rand(x.size()) - 0.5) * 2) * prop).cuda()  ##################################MNIST加性
...
#        cmask = torch.load('/home/user/zuoruichen/code/Motif/mask/CASNNTIDbp5-1/mask_before_sigmoid_1_0.torch')
